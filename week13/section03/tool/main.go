...
func MakeTool() openai.Tool {
	...
	params := jsonschema.Definition{
		...
		Properties: map[string]jsonschema.Definition{
			"instructor": {
				...
				Description: "The instructor of a course, e.g. Jane Doe",
			},
			"semester": {
				Type: jsonschema.String,
	...
	f := openai.FunctionDefinition{
		...
		Description: "Gets the courses taught by the given instructor",
		Parameters:  params,
	}
	t := openai.Tool{
...
func main() {
	...
	dialogue := []openai.ChatCompletionMessage{
		{Role: openai.ChatMessageRoleUser, Content: "What courses does Phil Peterson teach?"},
	}
	fmt.Printf("Asking OpenAI '%v' and providing it a '%v()' function...\n",
		dialogue[0].Content, tools[0].Function.Name)
